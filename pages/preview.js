...
import { CloseIcon, GpayIcon, PaytmIcon, PhonepeIcon } from "../components/Icons";
  ...
  return( amountList &&
    ...
    <Dialog.Portal>
      ...
      <Close>
        ...
        <ListContainer>
          {amountList && amountList.map((el) => {
            return(
              <ListItem onKeyDown={(e) => e.key === 'Enter' ? setValue(el) : null} tabIndex='0' onClick={() => setValue(el)} key={el}>
                {'\u20B9' + " " + el }
              </ListItem>
            )
          })  }
      ...
      <QRcodeImage src={img} alt={pa}/>
        ...
        <a href="/pratik" className="link" style={{alignSelf:'center', padding:'5px 20px', borderRadius:'10px'}}>
          <div className="flex " style={{gap:'10px', alignContent:'center', alignItems:'center'}}>
            ...
            <div style={{margin:'0px 10px', display:'flex', alignContent:'center', alignItems:'center', width:'30px'}}>
              <GpayIcon/>
            </div>
  ...
  const { pa, pn, amount_list, button_label } = router.query
    ...
    return ( 
        <div>
          ...
          {
            amount_list &&
       <Modal amounts={amount_list} pa={pa} pn={pn} button_label={button_label} />
